			...
			<div class="row row-centered">
			   ...
			   <div class="col-xs-12 col-centered col-min">	
					...
					<form  id="addtype" name="addtype" ng-submit="addtypesubmit()" novalidate >
					  ...
					  <div class="form-group has-feedback" ng-class="{ 'has-error' : addtypeedit.no_of_floor.$invalid && !addtypeedit.no_of_floor.$pristine }">
											...
											<p ng-show="addtypeedit.no_of_floor.$invalid && !addtypeedit.no_of_floor.$pristine" class="help-block">Field is required.</p>
					...
					<div class="row " ng-repeat="item in property_type_list " style="border: 1px solid #ecf0f5;padding: 11px 5px 17px 20px;background: #d1d7de;">
						  ...
						  <button class="btn btn-primary" ng-click="edit(item.id,item.title,item.no_of_floor)">Edit</button></div>
	...
	//alert($scope.title);
	 //alert($scope.productSelected.label);
        ...
        url : '<?php echo base_url();?>index.php/property/post_property_type',
				...
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
				data :JSON.stringify({title:$scope.title,no_of_floor:$scope.no_of_floor})
			}).success(function(data) {
				 console.log(data);
				// alert(data);
                if(data>0)
