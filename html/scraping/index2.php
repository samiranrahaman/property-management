<?php
/* $html = file_get_contents('https://play.google.com/store/search?q=apps&c=apps'); //get the html returned from the following url
...
if(!empty($html)){ //if any html is actually returned
	...
	$pokemon_row = $pokemon_xpath->query('//h2[@class="single-title-link"]');
	...
	//for ($i = 0; $i <= 1; $i++) 
{
  ...
  //echo $nodeList->item($i)->value . "<br/>\n";
	...
	//{
		if (!in_array($nodeList->item($i)->value,$url_array)) :
		$url_array[]=$nodeList->item($i)->value;
		//echo $nodeList->item($i)->value . "<br/>\n";
		echo $url2 = $i.'='.$k."="."https://play.google.com".$nodeList->item($i)->value;
		echo "</br>";
		$k++;
